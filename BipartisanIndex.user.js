...
function calculateSenate(){
  ...
  const origpageproto = window.location.href.split("vote_")[1].split("_")[0]+"_";
  const origpageproto2 = window.location.href.split(origpageproto)[1].split("_")[0];
  ...
  function getallpagespush (){
    ...
    for (let i=0; i<moreas.length; i++){
      if(moreas[i].getAttribute("href").includes("roll_call_votes") == true && moreas[i].getAttribute("href").includes("htm") == true){
      ...
      function senateavoid(reallinks){
        senarray.push(1);
        const thehref = reallinks.getAttribute("href").split(".htm")[0];
        const xmlhttp = new XMLHttpRequest();
        const pagelinksurl = thehref+".xml";
        xmlhttp.open("GET", pagelinksurl, true);
        xmlhttp.responseType = "document";
        xmlhttp.send();
        xmlhttp.onreadystatechange = function() {
